...
void BreamHelper::InjectTime(uint16_t year, uint16_t month, uint16_t day, uint16_t hour, uint16_t min, uint16_t sec, uint16_t accSec, uint32_t accNSec, uint8_t leapSec)
{
...
static bool OnNvmemOut(U1 *buf, U2 len)
{
    ...
    LD2_LOG("MGA-DBD : type:%d, offset:%d, total size:%d\n",
        dbtype, offset, size);
...
void BreamHelper::AddRawdataCb(LD2BRM_RawData cb)
{
    ...
    {
        LD2_LOG("%s() too many callback(%d) or same callback(%p,%p)\n",
