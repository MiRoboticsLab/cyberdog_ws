...
#
# 功能说明: 检测 environment 工具能力
#
function(judge_environment is_ok_)
  ...
  if("${_shyaml_info}" STREQUAL "")
    ...
    message("┏━> 检测 cyberdog_bringup 工作环境失败\n┠─> ${_error_info}\n")
...
function(generate_launch_file _file_name _file_data)
  ...
  file(WRITE ${_target_launch}
    "# This is the file generated by the ${PROJECT_NAME} project on the ${hostname} host at time ${COMPILE_TIME}.\n")
...
# 功能说明: 声明 parameters
#
macro(declare_parameters _target_file _target_key)
  if(_DEBUG_)
    message("┠─>: 正在声明 launch parameters...")
  endif()
  get_keys(${_target_file} "data.${_target_key}" _target_key_keys)
  list(FIND _target_key_keys "parameters" _parameters_key_index)
  if(${_parameters_key_index} GREATER_EQUAL 0)
    set(_parameters_key "data.${_target_key}.parameters")
    get_key_type(${_target_file} ${_parameters_key} _parameters_type)
    if(${_parameters_type} STREQUAL ${yaml_sequence_type})
      get_length(${_target_file} ${_parameters_key} _parameters_size)
      if(${_parameters_size} GREATER 0)
        math(EXPR _parameters_size "(${_parameters_size}-1)")
        foreach(_parameters_index RANGE 0 ${_parameters_size} 1)
          get_keys(${_target_file} "${_parameters_key}.${_parameters_index}" _target_parameters_keys)
          list(LENGTH _target_parameters_keys _target_parameters_keys_size)
          if(${_target_parameters_keys_size} LESS 2)
            continue()
          endif()
          list(GET _target_parameters_keys 0 _include_parameters_key)
          if(${_include_parameters_key} STREQUAL ${include_label})
            continue()
          endif()
          list(APPEND _launch "${_tab_2}DeclareLaunchArgument(\n")
          set(_key_index 0)
          foreach(_target_parameters_key IN LISTS _target_parameters_keys)
            set(_parameter_key "${_parameters_key}.${_parameters_index}.${_target_parameters_key}")
            get_key_type(${_target_file} ${_parameter_key} _parameter_type)
            if(${_parameter_type} STREQUAL ${yaml_sequence_type})
              get_sequence(${_target_file} ${_parameter_key} _parameter_key_vector)
              set(_target_key_value "'${front_bracket}")
              set(_meta_key_index 0)
              foreach(_parameter_key_meta IN LISTS _parameter_key_vector)
                ...
                string(REGEX REPLACE "'(.*)'" "\\1" _parameter_key_meta "${_parameter_key_meta}")
                string(CONCAT _target_key_value ${_target_key_value} "${_comma}${_parameter_key_meta}")
              endforeach()
              string(CONCAT _target_key_value ${_target_key_value} "${back_bracket}'")
            else()
              get_value(${_target_file} ${_parameter_key} _parameter_key_value)
              string(REGEX REPLACE "'(.*)'" "\\1" _parameter_key_value "${_parameter_key_value}")
              set(_target_key_value "'${_parameter_key_value}'")
            endif()
            if(${_key_index} EQUAL 0)
              list(APPEND _launch "${_tab_3}name='${_target_parameters_key}', default_value=${_target_key_value},\n")
            else()
...
macro(add_node_parameters _target_key _target_key_size)
  ...
  foreach(_parameters_index RANGE 0 ${_target_key_size} 1)
    ...
    list(APPEND _launch "${_tab_4}'${_parameter_key0_label}': LaunchConfiguration('${_parameter_key0_label}'),\n")
  endforeach()
  list(APPEND _launch "${_tab_3}},\n")
endmacro()
...
macro(definition_node_parameters _target_node)
  ...
  get_key_type(${yaml_node} ${_parameters_key} _parameters_type)
  if(${_parameters_type} STREQUAL ${yaml_sequence_type})
...
#
# 功能说明: 定义 node arguments
#
macro(add_node_argument _target_file _target_key)
  if(_DEBUG_)
    message("┠─>: 正在定义 node arguments...")
  endif()
  get_keys(${_target_file} "data.${_target_key}" _target_key_keys)
  ...
  if(${_arguments_key_index} GREATER_EQUAL 0)
    ...
    get_sequence(${_target_file} ${_arguments_key} _arguments_vector)
    list(APPEND _launch "${_tab_3}arguments=${front_bracket}\n")
    list(APPEND _launch "${_tab_4}'--ros-args'")
    foreach(_arguments_meta IN LISTS _arguments_vector)
      string(REGEX REPLACE "'(.*)'" "\\1" _arguments_meta "${_arguments_meta}")
      string(REPLACE " " ";" _arguments_meta_list "${_arguments_meta}")
      set(_meta_index 0)
      foreach(_arguments_list_meta IN LISTS _arguments_meta_list)
        if(${_meta_index} EQUAL 0)
          set(_comma ",\n${_tab_4}")
        else()
          set(_comma ", ")
        endif()
        math(EXPR _meta_index "(${_meta_index}+1)")
        list(APPEND _launch "${_comma}'${_arguments_list_meta}'")
      endforeach()
    endforeach()
    list(APPEND _launch "\n${_tab_3}${back_bracket},\n")
  endif()
...
# 功能说明: 定义 node arguments
#
macro(definition_node_argument _target_file _target_key)
  if(_DEBUG_)
    message("┠─>: 正在定义 node arguments...")
  endif()
  get_keys(${_target_file} "data.${_target_key}" _target_key_keys)
  ...
  if(${_arguments_key_index} GREATER_EQUAL 0)
    ...
    if(${_usr_args_key_index} GREATER_EQUAL 0)
      set(_usr_args_key "${_arguments_key}.usr-args")
      get_sequence(${_target_file} "${_usr_args_key}" _arguments_vector)
      set(_meta_index 0)
      foreach(_arguments_meta IN LISTS _arguments_vector)
        if(${_meta_index} GREATER 0)
          set(_comma ",\n${_tab_4}")
        endif()
        math(EXPR _meta_index "(${_meta_index}+1)")
        string(REGEX REPLACE "'(.*)'" "\\1" _arguments_meta "${_arguments_meta}")
        list(APPEND _launch "${_comma}'${_arguments_meta}'")
      endforeach()
    endif()
    list(FIND _target_arguments_key_keys "ros-args" _ros_args_key_index)
    if(${_ros_args_key_index} GREATER_EQUAL 0)
      set(_ros_args_key "${_arguments_key}.ros-args")
      get_sequence(${_target_file} "${_ros_args_key}" _arguments_vector)
      list(APPEND _launch "${_comma}'--ros-args'")
      foreach(_arguments_meta IN LISTS _arguments_vector)
        string(REGEX REPLACE "'(.*)'" "\\1" _arguments_meta "${_arguments_meta}")
        string(REPLACE " " ";" _arguments_meta_list "${_arguments_meta}")
        set(_meta_index 0)
...
#
macro(definition_launch_argument _target_file _target_key)
  if(_DEBUG_)
    message("┠─>: 正在定义 launch arguments...")
  endif()
  # list(APPEND _launch "${_tab_3}launch_arguments=${front_bracket}\n")
endmacro()
...
macro(definition_node _target_node)
  ...
  foreach(_key IN LISTS _target_node_keys)
    ...
    elseif(${_key} STREQUAL "arguments")
      definition_node_argument(${yaml_node} ${_target_node})
    elseif((${_key} STREQUAL "emulate_tty") OR
        (${_key} STREQUAL "respawn"))
      list(APPEND _launch "${_tab_3}${_key}=${_key_value},\n")
    else()
      message("┠─>: 无法识别的选项: ${yaml_node}->data.${_target_node}.${_key}")
    endif()
  endforeach()
  list(APPEND _launch "${_tab_3}output='${now_output}',\n")
...
function(automatically_generate_module_launch_files)
  ...
  foreach(_now_launch IN LISTS _launch_data_keys)
    ...
    foreach(_now_node IN LISTS _nodes_value)
      ...
      declare_parameters(${yaml_node} ${_now_node})
  ...
  if(${_file_name_index} GREATER_EQUAL 0)
    ...
    if(${_file_name_type} STREQUAL ${yaml_string_type})
      ...
      if(${_target_file} MATCHES ".*(\.py)+$")
        ...
        declare_parameters(${yaml_launch} ${_target_launch})
        list(APPEND _launch
        "${_tab_2}IncludeLaunchDescription(\n"
        "${_tab_3}PythonLaunchDescriptionSource(${_launch_file_source}),\n"
        )
        definition_launch_argument(${yaml_launch} ${_target_launch})
        list(APPEND _launch "${_tab_2}),\n")
      else()
        message("┏━> 忽略 ${_launch_msg}\n┗━> file_name 非 python 格式(*.py)，请检查")
      endif()
...
function(automatically_generate_launch_files)
  ...
  if(_${PROJECT_NAME}_AMENT_PACKAGE)
    message(FATAL_ERROR "automatically_generate_launch_files() 必须在 ament_package() 之前调用")
  endif()
  if(_ARG_LOG)
